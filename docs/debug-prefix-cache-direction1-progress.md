...
**范式第 2 条（完整保留全部哈希映射关联关系）与 vLLM 的 lazy evict 机制根本冲突**。vLLM 的设计假设是"block 物理回收即 evict hash"，方向 1 要求打破这个假设，但打破后要么内存不释放（路径 A），要么表膨胀（路径 C），要么 hash 保不住（evict-on-rewrite）。
