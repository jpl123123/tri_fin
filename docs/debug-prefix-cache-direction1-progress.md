...
> - **建议阅读顺序**：第一节（任务约束）→ 第二节（理想范式）→ 第三节（当前状态与大思路）→ 第六节（关键发现）→ 第七节（根本矛盾）
...
**范式第 2 条（完整保留全部哈希映射关联关系）与 vLLM 的 lazy evict 机制存在冲突**。vLLM 的设计假设是"block 物理回收即 evict hash"，方向 1 要求打破这个假设，但打破后的具体后果（内存不释放 / PC ON 反而变慢 / hash 保不住）在不同实现路径下表现不同。如何调和这个冲突是方向 1 未解决的核心难题。
