# TriAttention + Prefix-Caching 兼容性修复：系统性指导与失败总结

> 本文档是 `fix/prefix-cache-direction1-keep-hash-on-reclaim` 分支的最终移交文档。
> 当前 HEAD = `5a917c4`（路径 C 初版：物理释放 + hash 保护）。
> 本分支的工作**未能解决问题**，本文档系统记录任务约束、方向 1 完整内容、理想运行范式、所有尝试与失败原因、关键发现、错误总结，供下一位工作者接手。
...
> 注：`other_code/vllm-ascend-releases-v0.18.0` 是 vllm-ascend 扩展包，vLLM 核心源码（如 `vllm/v1/core/block_pool.py`、`vllm/v1/core/kv_cache_manager.py`）未包含在该路径，需在运行环境查看。本工作中的 explore 子代理曾在运行环境读取过这些源码，结论记录在第五节。
...
这三条在 vLLM 的 lazy evict 机制下互相冲突——物理块释放进 free pool 后，vLLM 复用该块时会清掉 hash（详见第五节 5.1）。本工作的所有尝试都在尝试调和这个矛盾，均未成功。
...
本分支共尝试了 6 个版本的修复，全部失败。按时间顺序：

### 5.1 轻量版（commit `3743257`）
...
### 5.3 路径 C 初版（commit `5a917c4`）— 当前 HEAD

**改动**：回退路径 A 的 pin 逻辑，物理块照常 `free_blocks` 归还，但给每个被回收 block 打 `_triattention_hash_protected` 标记，patch `BlockPool._maybe_evict_cached_block` 跳过带保护标记 block 的 hash 清理。详见第六节。

**简单测试结果**：通过（TTFT/TPOT/prefix ratio 与基线一致，内存 ~21%）。

**真实负载结果**：与未修复时一致（duration 716538ms, TTFT 4859ms, TPOT 84ms），PC ON 仍比 PC OFF 慢（duration +64614ms, TPOT +8ms）。

**当前版本状态**：此版本代码本身没有 bug（简单测试通过、内存正常），但问题也很明显——在 120 条不同 prompt 的真实工作负载下，PC ON 相比 PC OFF 没有带来收益，反而更慢（duration +64614ms, TPOT +8ms）。具体的机理定位（为何 PC ON 反而变慢）留给下一位工作者去做，此处只记录实验事实。

**大思路判断**：尽管路径 C 初版在真实负载下表现不佳，但**大思路肯定要按照路径 C 来做**。理由：
...
路径 C 初版的问题不是思路错，而是在真实负载下的具体表现需要进一步排查和改进。下一位工作者应在路径 C 的框架上继续，重点解决真实负载下 PC ON 反而变慢的问题。

### 5.4 路径 C 修复版（commit `b5ad232`，已回退）
...
### 5.5 方向 1：chunk cap 最小下限（commit `b2dc8d8`，已回退）
...
### 5.6 关闭路径 C 默认开关（commit `9ec6135`，已回退）
...
- **120 条 10k bs16 真实负载**：路径 C 失败（PC ON 比 PC OFF 慢，见 4.4）

---

## 七、关键发现（explore 确认的源码事实）

### 7.1 vLLM BlockPool 的 hash 生命周期
...
### 7.4 路径 C 在真实负载下的实验事实

路径 C 在 120 条不同 prompt bs16 真实负载下，PC ON 相比 PC OFF：duration +64614ms、TPOT +8ms、TTFT -741ms（仅小幅下降，远不及基线的 -1255ms）。具体的机理定位（为何 PC ON 反而变慢、为何 TTFT 降幅不及基线）留给下一位工作者。

### 7.5 简单测试 vs 真实负载的差异

- **简单测试（20k bs7 两次相同请求）**：第二次请求 100% 命中 touch 把受保护块移出 free pool，走"安全分支"，stale 污染不发生
- **真实负载（120 条不同 prompt）**：每个 prompt 只来一次，受保护块等不到 touch，只能被改写。具体后果见 7.4 的实验事实，机理定位留给下一位

### 7.6 基线 PC ON 为什么快

基线 PC ON 靠共享 system prompt 前缀命中降 TTFT，靠 prefill 计算量减少让 decode 等待更短降 TPOT。vLLM 原生 LRU evict 控制表规模小，hash 管理开销 < 命中收益。

### 7.7 TriAttention PC OFF 为什么正常
...
- 路径 C（保护标记 + 释放）：简单测试通过、内存正常，但真实负载下 PC ON 反而比 PC OFF 慢（见 7.4 实验事实）
- evict-on-rewrite（改写时清）：hash 在改写前清除，但真实负载下第二次请求来不及 touch

**范式第 2 条（完整保留全部哈希映射关联关系）与 vLLM 的 lazy evict 机制存在冲突**。vLLM 的设计假设是"block 物理回收即 evict hash"，方向 1 要求打破这个假设，但打破后的具体后果（内存不释放 / PC ON 反而变慢 / hash 保不住）在不同实现路径下表现不同。如何调和这个冲突是方向 1 未解决的核心难题，留给下一位工作者。

**但大思路肯定要按照路径 C 来做**（理由见 5.3 的"大思路判断"）。路径 C 的"物理释放 + hash 保护"框架是同时满足范式第 2、4 条的唯一方案。路径 C 初版在真实负载下的具体问题需要进一步排查和改进，而不是放弃路径 C 换方向。

---

## 九、本工作中的错误总结（供下一位工作者警惕）
...
### 9.3 没有区分"TriAttention 本身"和"PC 交互"
...
3. 最后对路径 C 在真实负载下 PC ON 反而变慢的机理做了定位，但用户要求只记录实验事实、不做事先的机理总结（机理定位留给下一位）
