# TriAttention + Prefix-Caching 兼容性修复：移交文档
...
这三条在 vLLM 的 lazy evict 机制下互相冲突——物理块释放进 free pool 后，vLLM 复用该块时会清掉 hash（详见第七节 7.1）。本工作的所有尝试都在尝试调和这个矛盾，均未成功。
...
**此版本代码本身没有 bug**（简单测试通过、内存正常），但问题也很明显——在 120 条不同 prompt 的真实工作负载下，PC ON 相比 PC OFF 没有带来收益，反而更慢。具体的机理定位（为何 PC ON 反而变慢）留给下一位工作者去做，此处只记录实验事实（见第五节 5.4）。
...
- **真实负载（120 条不同 prompt）**：每个 prompt 只来一次，受保护块等不到 touch，只能被改写。具体后果见 5.4 的实验事实，机理定位留给下一位
...
- 路径 C（保护标记 + 释放）：简单测试通过、内存正常，但真实负载下 PC ON 反而比 PC OFF 慢（见 5.4 实验事实）
...
**但大思路肯定要按照路径 C 来做**（理由见 3.3）。路径 C 的"物理释放 + hash 保护"框架是同时满足范式第 2、4 条的唯一方案。路径 C 初版在真实负载下的具体问题需要进一步排查和改进，而不是放弃路径 C 换方向。

---

## 八、本工作中的错误总结（供下一位工作者警惕）
